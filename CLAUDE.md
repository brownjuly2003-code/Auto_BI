...
**Все задачи Phase 2 (2.1–2.8) реализованы.** S6 (граница фазы) пока НЕ объявляю: exit criteria фазы («не-инженер собирает через web UI обоими режимами…») требуют live-смоука serve+UI против реального GraceKelly/Superset-стенда — стенд на Mac недоступен с этой машины. Сделать при первом доступе: live-смоук обоих режимов + native filters вопрос (deferred F3) — после этого S6/ревью фазы.
