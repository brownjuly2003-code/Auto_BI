...
**Остаток Phase 1:** только **1.10** (подключение реального DWH + интроспекция + первый gaps report) — нужны read-only доступы к реальному DWH от пользователя; с ним же закрывается критерий «реальный кейс по своему DM». S6 (граница фазы) объявлять после 1.10. Push невозможен — у репо нет remote.
