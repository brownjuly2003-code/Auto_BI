...
**ТЕКУЩЕЕ (2026-06-13, позже): S6-РЕВЬЮ PHASE 2 ПРОЙДЕНО, P2 ЗАКРЫТ, ВЛИТО В MAIN.** Ревью субагентом code-reviewer (Fable) → `fable_audit_phase2.md`: **P1 нет, 1×P2 + 7×P3**, вердикт «закрыть P2 → merge». Инварианты 1–8 соблюдены, XSS нет (везде `textContent`), Store потокобезопасен. **F1 [P2] закрыт** (pytest 196): упавшая сборка оставляла сессию в APPROVED → каждый retry 409; теперь approve при `build_status=="failed"` пересобирает тот же approved spec (+ тест `test_failed_build_can_be_retried`). **P3-backlog (после merge, по приоритету владельца):** F2 регистрация сессии до start + старт без lock (гонка/зомби при LLMError), F3 реестр сессий без TTL/капа/DELETE, F4 SSE-генератор держит threadpool-поток после дисконнекта (нужен heartbeat), F5 мутирующие эндпоинты без auth/Origin-чека (localhost-only не задокументирован), F6 enrichment-правка роли может инвалидировать spec живой сессии (перевалидация на approve), F7 dbt sources игнорируют `identifier`, F8 селектор BI в UI декоративный. **Phase 3 НЕ начинать без явного «go»**; перед ней — вопрос native filters (deferred F3 Phase 1) и судьба «joins в IR» (в PLAN нет ни в одной фазе, см. наблюдение ниже).
