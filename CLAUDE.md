...
> **СОСТОЯНИЕ СЕЙЧАС (2026-07-06, P7 плана `new_plen_05_07_26.md`): ДЕМО-ЗАПИСЬ S16 СДЕЛАНА — README-GIF полного живого цикла записан и вмонтирован, ЖДЁТ ПРИЁМКИ ВЛАДЕЛЬЦА (локальный коммит, push после «да»).** Playwright-запись боевого `serve` (GraceKelly→Sonnet 5.0, реальный CH+Superset через туннель): «средний чек по месяцам + Парето по магазинам + динамика количества» → уточнение «количество: orders/items?» → ответ → spec (ratio-мера `revenue/orders` grain=month, `running_share`-Парето по `dm.stores.name`, orders по дням) + CRITICAL-вердикт advisor (скан 100% из 20M строк) в кадре → сборка 4 с → дашборд Superset #36. Ассеты: `docs/screenshots/demo.gif` (3,1 МБ, 47 с, паузы LLM сжаты таймлапсом) + `demo.mp4` (0,8 МБ), README §Демо обновлён. Инструменты записи/монтажа: `D:\dashboard_kit\_ab_demo_record.mjs` + `_ab_demo_cut.py` (вне репо). **Гочи сессии:** (1) кириллица через argv curl/Git Bash умирает в `?` (ANSI-codepage) — тела запросов ТОЛЬКО из UTF-8-файлов (`--data-binary @file`); (2) стенд был задушен осиротевшим kind-кластером `hq-demo` (AgentFlow E4, 200% CPU) + разогнавшимся `datalens-temporal` (257% CPU) — CH-запросы 25-90 с, build падал по TIMEOUT_EXCEEDED; после `docker stop` обоих — агрегат 0,4 с, build 4 с. hq-demo ОСТАВЛЕН остановленным (вернуть: `docker start hq-demo-control-plane hq-demo-worker hq-demo-worker2`), datalens-стек перезапущен обратно; (3) относительный `dashboard_url` (`/superset/dashboard/N/`) из UI не кликается при разнесённых хостах — кандидат в P10. Статусы P1–P6/P9 — в `new_plen_05_07_26.md`. **ОТКРЫВАЕТ:** P10 (после приёмки P7); P8 опционален.
>
