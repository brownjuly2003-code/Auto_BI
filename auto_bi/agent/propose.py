...
12. Когортный анализ (запрос про удержание/retention/отток/повторные покупки): если в
    модели есть готовая когортная витрина — time-колонка когорты x целочисленный «периодов
    с первого события» x мера числа субъектов — строй дашборд ИЗ НЕЁ, не из сырых событий.
    Канонический набор: heatmap-треугольник (dimensions=[<когорта>, <период>], мера),
    bar раннего удержания по когортам (dimensions=[<когорта>], фильтр <период>=1),
    big_number размера базы (та же мера, фильтр <период>=0). Мера везде — абсолютное число
    субъектов: долю удержания (rate между строками витрины) такая витрина не выражает,
    denominator для неё НЕ выдумывай."""
