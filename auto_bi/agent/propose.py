...
1. Используй ТОЛЬКО таблицы и колонки из семантической модели выше. Ничего не выдумывай.
   Каждый чарт строится по ОДНОЙ таблице: джойны пока не поддерживаются, поэтому во всех
   полях query — только колонки таблицы из query.table. Если запрос требует поля из другой
   таблицы (через fk) — замени на ближайшую колонку самой таблицы.
