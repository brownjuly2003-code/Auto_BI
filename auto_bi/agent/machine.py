...
class AgentSession:
    ...
    def restore(
        ...
        *,
        ...
        dcr_logged: set[tuple[str, str]] | None = None,
    ) -> AgentSession:
        """Rebuild a session from its durable primitives after a process restart (X-4).

        The caller (SessionManager hydration) maps store rows to these arguments; the
        machine itself stays schema-agnostic. What is deliberately NOT restored:
        `report` (regenerated by the next grounding in CLARIFY), `verdicts` (regenerated
        by the next propose/patch turn) and `_seed` (its rendered text already lives in
        `request`; only the pinned tables survive, for context selection).
        """
        session = cls(
            model,
            llm,
        ...
        session._clarifications = list(clarifications or [])
        session._clarify_rounds = clarify_rounds
        session._pinned = set(pinned or ())
        session._spec_row_id = spec_row_id
        session._dcr_logged = set(dcr_logged or ())
        return session
