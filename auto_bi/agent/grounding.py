...
3. Конкретный период в запросе («июнь 2026», «за 2025 год», «последние 30 дней») —
   это фильтр по time-колонке модели, НЕ сущность: при наличии time-колонки период
   не сверяется с примерами значений и не попадает ни в ambiguous, ни в unmatched.
4. Если запрос однозначен — ambiguous и unmatched оставь пустыми. Лишние уточнения вредны.
5. Колонки с одинаковым смыслом в разных таблицах («название» есть и в товарах,
   ...
   лишний вопрос хуже разумного выбора по контексту. НО «Выручка по названию» таблицу
   названия НЕ определяет (выручка режется и по товарам, и по магазинам) → ambiguous
   с name-колонками обеих таблиц.
6. Расплывчатое слово меры, под которое подходят НЕСКОЛЬКО колонок role=measure с
   разным смыслом, — ВСЕГДА ambiguous, даже если одна мера кажется вероятнее по
   бизнес-конвенции и все кандидаты лежат в одной таблице (контекст таблицы тут
   ничего не решает). Так, «количество» при мерах и заказов (orders), и штук (items)
   обязано стать вопросом:
   {{"ambiguous": [{{"phrase": "количество",
   "candidates": ["dm.sales_daily.orders", "dm.sales_daily.items"],
   "note": "заказы или штуки"}}]}}
   Выбирать самому можно, только когда запрос сам уточняет меру («число заказов»,
   «количество товаров (qty)» — явное имя или однозначное слово снимает вопрос).
7. Производные метрики-отношения («средний чек» = выручка/заказы, «маржа» =
