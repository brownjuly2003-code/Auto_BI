...
2. Слова о форме подачи («график», «топ-10», «по дням», «дашборд») — НЕ сущности данных:
...
5. Производные метрики, требующие деления одной меры на другую («средний чек» =
   выручка/заказы, «конверсия», «доля рынка»), которых НЕТ готовой мерой в модели, —
   это unmatched: вычислять отношения мер дашборд не умеет, не подменяй их похожей
   мерой. (Доли в pie-чарте — форма подачи, НЕ производная метрика.)
