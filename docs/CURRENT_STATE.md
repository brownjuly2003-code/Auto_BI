...
| Mypy --strict allowlist | `auth.py`, `adapters/artifacts.py`, `errors.py`, `config.py`, `ir/validate.py`, `ir/spec.py`, `semantic/model.py` (CI; grow via SLO.md) |
...
6. **Step 12 residual** — live SLO; extend mutation coverage beyond SQL guard;
   grow the seven-module mypy-strict allowlist.
