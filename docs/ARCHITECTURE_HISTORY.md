# Auto_BI — История архитектуры

> Этот документ — архив эволюции дизайна: исходные допущения, хронология
> поставки, номера внутренних задач и доказательства со стендов. Актуальные
> компоненты и обязательные инварианты находятся в
> [ARCHITECTURE.md](ARCHITECTURE.md).
>
> Смежные поверхности имеют отдельные роли:
> [CURRENT_STATE.md](CURRENT_STATE.md) фиксирует текущее состояние продукта,
> [PLAN.md](PLAN.md) хранит фазовую историю, а [adr/](adr/) — принятые решения
> и их последствия. Этот архив не является источником текущего статуса.
>
> Это frozen pre-split snapshot. Номера «инвариант N» внутри архива были
> неформальными и не сопоставляются с каноническим списком в
> [ARCHITECTURE.md §2](ARCHITECTURE.md#2-ключевое-решение-ir-first-dashboard-spec).
> При конфликте секций всегда побеждают current design и ADR. В частности,
> legacy-описание `set_query_plans` в §3.19 superseded обязательным
> `BuildContext.plans` из
> [ADR 0001](adr/0001-bi-adapter-contract.md).
...
tables:
  - name: dm.sales_daily
    ...
    columns:
      ...
      - {name: return_rate, type: numeric, role: measure, agg: avg, additivity: non_additive}
    physical:
...
- **`additivity` у measure-колонки (P1-6, 2026-07-17)** — семантика суммирования: `non_additive` (rate/ratio/цена за единицу — построчный SUM бизнес-бессмыслен), `semi_additive` (аддитивна не по всем осям: distinct-count-снэпшот, остаток — записывается как намерение моделирования, в v1 НЕ энфорсится), `additive` (явное «sum корректен»); не задано → без ограничений (обратная совместимость). Governance исполняемый, три слоя одного правила: интроспектор размечает rate-подобные имена (`rate|ratio|pct|percent|share`, `price`/`unit_price` — целыми токенами, `introspect/base.py::rate_like`) как `agg: avg` + `non_additive`; валидация спеки отклоняет `sum` над `non_additive` (включая denominator ratio-меры) с подсказкой для repair loop; enrichment-API отвечает 422 на попытку руками поставить такой `sum`. Autospec без модельного `agg` для `non_additive` падает в AVG (не SUM) и не строит share-of-total над неаддитивной primary-мерой (доля от суммы средних — не part-to-whole). Маркер рендерится в model_text — LLM видит запрет до, а не после отклонения.
- **`physical.captured_at` (P1-6)** — UTC-штамп снятия статистики интроспектором: `rows`/`cardinality` замораживаются в git, а окружения расходятся (демо 1M / стенд 20M / прод 100M) — штамп делает устаревание видимым. Потребитель, который может измерить вживую, не должен верить снэпшоту как «сейчас» (advisor §3.3: live-знаменатель доли скана).
- **`synonyms` у таблицы и колонки (X-3, 2026-07-11)** — рукописные альтернативные имена («удержание»/«retention»/«отток» у `dm.cohort_retention`): пользователь спрашивает своими словами, не именем витрины. Слой данных, не промптов: рендерятся в model_text (`[синонимы: …]`, НЕ гейтятся `include_samples` — это словарь моделирования, не сэмплы данных) и скорятся контекст-селекцией с весом ИМЕНИ (таблица 3.0 / колонка 2.0 — синоним и есть альтернативное имя), так что на большом DM витрина не выпадает из 40k-бюджета по запросу, в котором нет ни одного её физического слова. Интроспектор их НЕ генерит — словарь, как и description, решение моделирования (enrichment-workflow).
...
**Когортный паттерн в PROPOSE (X-3, 2026-07-11).** SPEC_RULES п.12: запрос про удержание/отток при наличии в модели готовой когортной витрины (форма: time-когорта × целочисленный «периодов с первого события» × мера числа субъектов) собирается ИЗ НЕЁ, не из сырых событий; канон = heatmap-треугольник + bar раннего удержания (фильтр период=1) + big_number размера базы (фильтр период=0); мера всюду абсолютная (rate между строками витрина не выражает — правило явно запрещает выдумывать denominator). Паттерн распознаётся по ФОРМЕ витрины, не по именам (domain-agnostic); словарный мостик «своими словами → витрина» — synonyms §3.2. Live-verified 2026-07-11: `build "Покажи удержание клиентов" --target datalens` на стенде → grounding нашёл витрину только через синонимы → spec = канонической тройке → 3/3 чартов с данными (`/api/run`); heatmap на DataLens деградирует в pivot (документированная деградация §3.4). Деталь: propose пиннит только seed-таблицы (fields-first), grounding-таблицы НЕ пиннятся — контекст-селекция propose скорит сырой запрос заново, поэтому синонимы обязаны работать в ОБОИХ шагах (они в общем `select_context`/`render`).
...
**Effective query — что именно судит advisor (P1-2, 2026-07-17).** Правила и EXPLAIN видят не дословный `query` спеки, а **effective query**: фильтры чарта **плюс дашбордные контролы, которые реально сужают этот чарт** (`advisor/effective.py`). До Phase 2 дашбордные фильтры не компилировались, и чтение одних `query.filters` было честным; с приходом native-фильтров (§3.5) оно стало давать ложный `no_filter_on_large_fact` на спеке, где период задан только контролом. Контрол засчитывается по тем же двум условиям, что кодируют оба адаптера: колонка входит в grain чарта (иначе агрегированный датасет нечем фильтровать) **и** у контрола непустой `default` (пустой → нейтральная маска → чарт открывается несуженным). Засчитывать все `spec.filters` скопом нельзя — это поменяло бы ложный позитив на ложный негатив, то есть молчание на настоящем full scan. Тип контрола берётся из `ColumnRole` модели, а не из `DashboardFilter.type` (LLM ненадёжно переопределяет его schema-дефолт). Фильтр вешается на **собственную ссылку чарта** на колонку: scope матчится по голому алиасу, и контрол, объявленный против другой таблицы, иначе утёк бы её именем в SQL чарта.

**Live-знаменатель доли скана (P1-6, 2026-07-17).** `explain_high_scan_fraction` делит live `EXPLAIN ESTIMATE` на размер таблицы — и знаменатель обязан быть таким же живым, как числитель: статический `physical.rows` заморожен в git и расходится с окружением (модель 20M vs демо 1M vs compose 100M), давая ложную долю или >100% без единого лишнего прохода. Когда advisor'у доступен RunQuery (он только что прогнал EXPLAIN через тот же шов), он берёт текущий размер из `system.tables` (`advisor/explain.py::live_row_count`, один каталожный запрос на таблицу на инстанс, never-raise) и кладёт его в evidence как `live_total_rows`; правило предпочитает его, `total_rows_source: live|model` фиксирует происхождение. Фолбэк на модельный `rows` — при недоступном каталоге, metadata-only режиме и live-нуле (пустая/detached таблица — не сигнал для знаменателя). GP-пак scan-fraction-правила не имеет — там без изменений; метаданные-правила (`no_filter_on_large_fact`, severity по размеру) сознательно остаются на модельном `rows`: они судят дизайн-намерение витрины, не текущий момент.

**Advisor во всех entrypoints (P1-2).** Раньше детерминированные находки существовали только в чат-пути: auto-обзор (`cli._build_auto`, `machine.adopt_spec`) и одношаговый text-CLI (`pipeline.build_dashboard`) шли в сборку мимо advisor'а — реальная находка просто терялась. Теперь advisor работает во всех трёх. В путях без диалога вердикты **механические** (`worst_verdicts` — текст самих правил, без narration-вызова): вердикт по D5 всё равно выносит код, LLM отвечает только за формулировку, а лишний вызов провайдера на каждую сборку — это его квота ради переписывания текста, который инженерный CLI читает как есть. `dm_change_request` логируется в store одинаково на всех путях — заявка решена правилами и не зависит от того, нарратил ли кто-то её текст.
...
- `query` — декларативный (таблица/измерения/меры/фильтры), не сырой SQL: SQL генерируется отдельным шагом и валидируется. Эскейп-хэтч `raw_sql` допускается, но помечается и проходит sqlglot-guard (реализован — §3.16).
...
**Правила стабильности DataLens-адаптера (Phase 4 F7, инвариант 7 распространён на DataLens):** реверс-блобы (zod `dataSchema` `schemeVersion=8`, chart `shared` `version="4"`, charset имени entry, `mix/createDashboardV1`/`mix/deleteEntry`, gateway `v4.10.4`, `HC=1`) завязаны на конкретную версию self-hosted стенда. Поддержанная версия и контрактные маркеры зафиксированы в runbook `internal/2026-06-13-datalens-selfhosted-runbook.md` (секция «Версия стенда — контрактный пин»); обновление версии стенда — отдельная задача с обязательным прогоном live contract-сьюта `tests/test_datalens_contract.py`. Гэп: точные image-digest'ы (стенд = depth-1 клон, плавающий тег) ещё не сняты — команда захвата в runbook.
...
- **Bar над временной осью держит time-ось (2026-07-11).** Каст числового измерения в категориальную ось (`xAxisForceCategorical`) чинит `store_id`, но ЛОМАЛ bar над TIME-колонкой (когорты по `cohort_month`): категориальная ось печатала сырой epoch-ms бакета («1769904000000» вместо «июл 2024»). Теперь `build_form_data` различает temporal-x (`time_column == alias(x_axis)` — колонка роли TIME И сама ось X): такой bar НЕ форсится категориальным, остаётся на time-оси и получает явный формат меток `x_axis_time_format="smart_date"` (адаптивный дата-формат Superset; для line это no-op — линия и так на time-оси). Числовой non-temporal x (`store_id`) форсится категориальным как раньше. Гейт: юнит + Superset contract 22/22 (round-trip form_data на живом 4.1.2). Симметрично DataLens-касту числового измерения (B2, выше). **Хвост heatmap_v2 ЗАКРЫТ тем же днём (2026-07-11 вечер):** ряд `months_since=0` метился `<NULL>` на Y-оси (upstream #33105 — js-препаратор heatmap_v2 глотает numeric 0/false как «пусто»; #31318 — alpha-сортировка числовых ключей), данные при этом корректны. Обход вкожен: `build_form_data(heatmap_y_pad=N)` подменяет groupby на adhoc-колонку `LPAD(CAST(y AS VARCHAR), N, '0')` (ANSI — рендерится и на CH, и на GP) — «00»…«23» чинит и метку нуля, и порядок разом. Ширину вычисляет адаптер (`_heatmap_y_pad`): ТОЛЬКО числовое DIMENSION с кардинальностью ≤100 (ordinal-периоды когорт, где нулевой ряд — норма), `width = digits(card-1)`, min 2; id-подобная ось (store_id, тысячи значений) остаётся с естественными метками — нулевого ряда там нет, а «0001» строго хуже читается. Верификация: гипотеза «0 → falsy» подтверждена живым probe-чартом ДО правки кода (скрины до/после), затем e2e самим адаптером на стенде (pad=2 вычислен из model_stand, чарт с adhoc groupby, скрин «00..23»), contract 22/22 на демо (контрактный heatmap date×store_id — старый путь, без регресса); чарты витринного дашборда 40 стенда допатчены теми же шаблонами (heatmap + bar time-ось).
...
только транспорт. Каждый клиент импортируется лениво — выбор GraceKelly не импортирует
Anthropic SDK в рантайме (хотя пакет стоит как core-зависимость).

**Anthropic (по умолчанию, `llm/anthropic.py`)** — прямой Messages API, без внешнего сервиса:
нужен только ключ (`ANTHROPIC_API_KEY` или `AUTO_BI_ANTHROPIC_API_KEY`). SDK — **core**
dependency (`pip install autobi-agent`, production Docker; audit P1-3), не optional extra.
Убирает единственную точку отказа стороннего сервиса — воспроизводимость у внешнего
пользователя не зависит от чужой локальной инфраструктуры. `thinking:{type:"adaptive"}`
на reasoning-шагах (зеркало `reasoning: true` у GraceKelly), `{type:"disabled"}` на
механических. Реальные токены usage (`response.usage.input_tokens/output_tokens`)
захватываются в store (§3.9) — GraceKelly их не возвращает, поэтому токен-метрики есть
только на этом пути.
...
**Бюджет на шве клиента (P0-3 item 4, `llm/budget.py`).** HTTP-квоты (`config.py`: session_rate
O-2, work_rate P0-3) гейтят *запросы*, но не видят реальных обращений к провайдеру — один HTTP-запрос
разворачивается в grounding + propose + narrate advisor + до 3 репэйр-повторов. `LLMBudget`
энфорсит лимит там, где эти обращения реально происходят: `_structured.complete_with_repair`
зовёт `LLMBudget.check` через хук `on_attempt` **перед каждой** попыткой, поэтому первичный вызов И
каждый репэйр списывают бюджет, и обойти шов нельзя (проверка живёт в единственном месте, где
дёргается `send_fn`). Fail closed — `BudgetExceeded(LLMError)` поднимается **до** вызова, который
пересёк бы лимит, с указанием измерения. Два скоупа, оба на каждый вызов: **сессия** (весь диалог)
и **актор / скользящие 24ч** (владелец сессии при auth on; единый глобальный бакет при auth off —
предохранитель суммарного расхода для анонимного демо). Измерения: вызовы, токены (реальные у
Anthropic, оценка `chars/4` у GraceKelly), время (кумулятивная латентность провайдера), стоимость
(таблица цен $/1000 токенов на модель; опционально). Источник агрегата — существующий леджер
`llm_calls` (§3.9): каждая попытка уже пишется туда `_call`'ом, поэтому репэйры считаются сами собой
и бюджет переживает рестарты/запросы без параллельной таблицы. Конструируется в `make_llm` из
`AUTO_BI_LLM_BUDGET_*`. **Opt-in, по умолчанию выключен** (как session/work-квоты): CLI/тесты/локалка
не затронуты; LLM-демо включает его явно (см. DEPLOYMENT §3). Инварианты 1–8 не затронуты — шов
целиком на LLM-слое, IR/`BIAdapter` не меняются.
...
**Retention (D-3, 2026-07-18).** Таблицы делятся на две породы: работа пользователя (`sessions`/`messages`/`specs`/`builds`) и телеметрия, растущая с использованием (`llm_calls`, `trace_events`, не-`live` строки `bi_artifacts`). `Store.purge_retention` подрезает по возрасту **только вторую** — первая живёт, пока её не удалят осознанно. `live`-строки леджера не стареют ни при каком сроке: по ним селектится ownership-cleanup (§3.17), и их удаление осиротило бы реальный артефакт в BI. Выключено по умолчанию (`AUTO_BI_RETENTION_ENABLED`) — удаление операционной истории необратимо, поэтому включается оператором осознанно, как и остальные ограничители. SQLite не возвращает место ОС после DELETE — файл ужимает только ручной `VACUUM`.
...
Токен-учёт **реализован** на Anthropic-пути (аддитивные nullable-колонки `llm_calls`, без смены модели данных — ровно как и планировалось). **$-стоимость** считается поверх токенов по прайс-таблице `AUTO_BI_LLM_BUDGET_PRICES` (владелец цен появился вместе с бюджетным guard'ом): одна функция `llm.budget.cost_usd` обслуживает и cost-лимит, и метрику spend, поэтому обе стороны оценивают один леджер одинаково. Незалистанная модель тарифицируется в **0** — то есть cost-лимит на ней молча не сработает; модель добавляют в таблицу до того, как на лимит полагаются.

- **Prometheus-эндпоинт (D-3, 2026-07-18)** — `GET /api/v1/metrics`, text exposition, opt-in `AUTO_BI_METRICS_ENABLED`. Дополняет две панели выше третьим срезом: не «что было в этой сессии», а «что происходит с процессом» — билды по статусу и in-flight, слоты сборки, DWH-проходы и время в них, LLM-вызовы/токены/секунды/стоимость, число строк по таблицам store. Два источника намеренно разделены: `LiveMetrics` — счётчики процесса (сбрасываются на рестарте, обычный counter reset), `Store.metrics_snapshot()` — durable-агрегаты, читаются на каждый scrape. Рендер свой (модуль `api/metrics.py`, 174 строки), без `prometheus_client` (его глобальный реестр и default-коллекторы в библиотеке, которая ещё и CLI, — лишние). Цифры **глобальные**, поэтому, в отличие от `/observability/llm`, деградации до per-owner-вида нет: при включённом auth эндпоинт admin-only.
...
- **Что гоняется живьём:** `pytest -m integration tests/test_superset_contract.py tests/test_d1_acceptance.py` (create→GET→assert + chart/data + native-filters roundtrip по всем `viz_type`; D-1 acceptance — filterable SOURCE dataset, period + `stores_name` re-scope vs ClickHouse, §3.20), затем `auto_bi build --auto dm.sales_daily --target superset` — детерминированный auto-overview путь (без LLM, ARCHITECTURE §auto-overview), так что живой E2E-прогон не требует `ANTHROPIC_API_KEY`/секретов вообще. CH/Superset-пароли — дефолты `docker-compose.yml` (`change_me`), достаточно для эфемерного, непубликуемого CI-стенда.
- **Браузерный E2E + axe (D-4):** после CLI-пути тот же job гоняет `tests/test_web_e2e.py` — настоящий Chromium (Playwright) против `auto_bi serve` в публичном demo-профиле (`AUTO_BI_DEMO_AUTO_ONLY`: DisabledLLM, ни ключа, ни трат) поверх того же стенда. Путь пользователя целиком: вкладка «Авто» → выбор витрины → «Собрать обзор» → превью спеки → «Собрать дашборд» → SSE-лог → ссылка на живой дашборд (GET 200). Каждое состояние UI по дороге сканируется axe-core — статические D-5-тесты (`tests/test_ui_a11y.py`) пиннят конкретные починки, axe покрывает всю отрендеренную страницу. `playwright`/`axe-playwright-python` подтягиваются эфемерно (`uv run --with`, как pytest-cov/duckdb в `quality`), маркер `e2e` деселектнут по умолчанию наравне с `integration`.
- **DataLens вне скоупа этого job'а** (в отличие от `test_superset_contract.py`) — самостоятельный self-hosted стенд с ручным реверс-инжинирингом (см. §3.5/`internal/2026-06-13-datalens-selfhosted-runbook.md`), в `docker-compose.yml` не описан; `test_datalens_contract.py` по-прежнему Mac-only.
...
- **`release.yml`** — триггер только на push тега `vX.Y.Z`. Порядок (plan_sol
  шаг 6): `preflight` → параллельно `image-security` (build local → Trivy →
  image SBOM → push `:version` → attest), `pypi`, `provenance` → `finalize`
  (`:latest` + GitHub Release с source/image SBOM + changelog section).
  Mutable `:latest` и Release не появляются до зелёных gates.
...
Начинает закрывать T-2 из `audit_fable_03_07_26.md` — golden-сьют (§1.11) диалоговый, гоняется через реальный `LLMClient` и раньше запускался только вручную с живым провайдером; в CI шла только детерминированная advisor-часть (`quality`-job, §3.3/invariant 8).

- **Контракт** — `auto_bi/llm/fixture.py`: `FixtureLLMClient` (replay / refresh-fingerprints) и `RecordingLLMClient` (record) оба структурно удовлетворяют протокол `LLMClient` (`complete(prompt, schema, ...) -> T`), поэтому подключаются на ТОМ ЖЕ шве, что `GraceKellyClient`/`AnthropicClient`, без нового интерфейса ни в `agent/`, ни в `eval/runner.py`.
- **Fixture format v2 (plan_sol step 9):** один JSON на кейс, `<fixtures_dir>/<case_id>.json` — header `format_version` / `template_version` / `schema_version` / `provider` / `model_id` + `calls[]` с `step`, `schema`, **`prompt_sha256`**, `response`. `template_version` = hash статических GROUNDING/SPEC_RULES/PROPOSE/PATCH; `schema_version` = hash JSON Schema `GroundingReport`+`DashboardSpec`; `prompt_sha256` = sha256 нормализованного outbound prompt. Несовпадение → `FixtureStaleError` (не silent reuse).
- **Кто знает текущий кейс** — общий `llm`-объект один на весь прогон сьюта; `run_golden_case` дак-тайпит `begin_case`/`end_case`.
- **Расхождение call sequence** — `FixtureMissingError` (другой step/schema/число вызовов). **Расхождение prompt contract** — `FixtureStaleError`.
- **Пороги (честный split):** replay / refresh-fingerprints = **100%** кейсов; live / record = clear ≥ 80%, ambiguous/infeasible — all-or-nothing (`golden_suite_ok(mode=…)`).
- **Что replay ловит, а что нет.** Оффлайн-replay + fingerprints: обвязка вокруг LLM **и** актуальность fixture к текущему prompt surface. **Не** ловит живую деградацию модели — для этого live sentinel (`.github/workflows/eval-live-sentinel.yml`, 3 кейса, budget cap, skip без секрета) или ручной `--llm-mode live`. Процедура re-record: `docs/EVAL_FIXTURES.md`.
- **CLI** — `auto_bi eval --suite golden --llm-mode {live,replay,record,refresh-fingerprints}`. `refresh-fingerprints` офлайн переписывает hashes/meta, ответы не трогает.
- **CI:** advisor CH + advisor GP; golden replay CH + GP (fingerprint enforced). 53 fixture-файла (37 CH + 16 GP) в `tests/fixtures/golden_llm/`.
...
### 3.16 Escape hatch `raw_sql` (X-5)

Осознанное исключение из инварианта 1 для запросов, которые IR не выражает (экзотическое окно,
конструкция без IR-примитива). `ChartQuery.raw_sql` несёт готовый `SELECT`; когда он задан, SQL_GEN
возвращает его ДОСЛОВНО (диалект не перекомпилирует — SQL уже написан под целевой движок), и дальше
он проходит РОВНО тот же live-гейт, что и сгенерированный SQL: `guard_sql` (единственный plain
`SELECT`, запрет DDL/DML) → EXPLAIN → LIMIT-прогон (`compile_and_build`).

Это **ручной люк** (CLI `auto_bi raw`), НЕ LLM-генерация: инвариант 1 («LLM выдаёт только IR»)
сохраняется — сырой SQL пишет оператор, а не модель. NL→SQL в ядро не берётся осознанно
(ломает D1/D5 и ров: ~6% тихо-неверного SQL для BI хуже отказа — `nl_sql_decision_03_07_26.md`).

**Provenance / LLM-граница (P0-1, 2026-07-16):** runtime-IR по-прежнему несёт `ChartQuery.raw_sql`
(CLI hatch), но PROPOSE/PATCH видят JSON Schema **без** этого поля (`llm_dashboard_spec_schema`)
и валидируются с `allow_raw_sql=False` — любой `raw_sql` из text/fields/repair → error. Schema-RBAC
(`auth.spec_tables` / `forbidden_tables`) для raw ходит по AST SELECT (`extract_table_names`),
а не только по `query.table` (метка датасета): `FROM finance.secret` при label `dm.allowed`
отклоняется для пользователя со схемой `[dm]`. `guard_sql` дополнительно режет table-function
remote-источники (`url`/`s3`/`remote`/…).

Границы (ров ослаблен осознанно, и это отражено в коде):
- только `viz=table` — сырой результат ложится в таблицу без IR-маппинга колонок на оси;
  `dimensions`, если заданы, — имена колонок результата для показа (Superset `query_mode=raw` +
  `all_columns`), иначе показываются все колонки;
- нельзя смешивать с агрегирующими IR-полями (`measures`/`series`/`rows`/`columns`/`joins`/
  `filters`/`order_by`/`time_grain`/`bins`) — raw-чарт несёт весь запрос в SQL (`validate_spec`);
- **Feasibility Advisor слепнет** на raw-чарте (он рассуждает над IR), нормализации B1/B3
  (top-N/label-join) и форматные слои НЕ применяются — `validate_spec` не может проверить колонки
  по модели (в этом и смысл эскейп-хэтча);
- `ChartQuery.table` для raw — только метка для имени датасета, в модели существовать не обязана
...
### 3.19 Общий план запроса (D-2, 2026-07-19)

Guard и Advisor оба планируют SQL чарта на DWH, из модулей, которые друг о друге не знают:
guard — `EXPLAIN <sql>` (доказать, что стейтмент резолвится), Advisor — стейтмент evidence
своего движка (`EXPLAIN ESTIMATE` на ClickHouse, обычный `EXPLAIN` на Greenplum). На
одношаговых путях сборки (`auto_bi build`, `build --auto`) review и build идут подряд, и
второй проход не измеряет ничего нового.

`PlanCache` (`agent/query_plan.py`) — мемо на **один вызов сборки**. Два независимых
хранилища с одним ключом (точный текст SQL):

1. **Plan entries** (D-2 §3): Advisor записывает результат своего плана; guard **читает**
   и пропускает собственный `EXPLAIN`, если этот же стейтмент уже спланирован без ошибки.
   First-record-wins. Guard plan entries **не** пишет.
2. **Trial entries** (D-2 §5): guard после LIMIT-прогона (`TRIAL_LIMIT = 10`) записывает
   полученные строки и флаг `complete = len(rows) < TRIAL_LIMIT`. First-record-wins
   независимо от plan entry — plan и trial для одного SQL сосуществуют. Неполные
   (ровно 10 строк) результаты **не** переиспользуются: ORDER BY по усечённому набору —
   ловушка «млн вместо млрд».

Кэш не ходит в DWH сам и не импортирует ни advisor, ни guard — слои однонаправленные.

**Кто пишет / кто читает trial.** Guard записывает (равномерно для каждого
провалидированного стейтмента, включая source SQL). Потребители:

- **Superset magnitude** (`_measure_magnitude`): только OWN-чарты (`from_source=False`),
  complete trial, колонка `measure_alias` с хотя бы одним float → `max` вместо
  `/api/v1/chart/data`. SOURCE **всегда** проба (после D-1 guard gate'ит shared source
  SQL — сырые строки витрины, не агрегат чарта; выводить magnitude из 10 raw-строк
  нельзя).
- **DataLens magnitude**: то же правило минус `from_source` (на этой lane каждый чарт
  gate'ится своим SQL). Сигнатура `_measure_magnitude` принимает `chart` для ключа SQL.
- **CLI insights** (`analyze_spec(..., plans=...)`): complete trial → reuse вместо
  `run_query`. `serve /insights` **не** получает кэш (отдельный запрос; персист строк
  между запросами вне scope).

Wiring: `compile_and_build` при `plans is None` создаёт свежий build-local `PlanCache`
(пустой — без advisor evidence, EXPLAIN-skip семантика API-пути не меняется) и передаёт
его адаптеру через concrete helper `set_query_plans` (как `set_artifact_namespace`;
`BIAdapter` Protocol не тронут).

**Ключ — точный текст SQL, и это несущее.** Plan-потребители смотрят на РАЗНЫЕ запросы:
Advisor судит *effective query* (фильтры чарта + контролы дашборда, P1-2) по
**до**-нормализационной спеке, guard — **после**-нормализационную (B3 заменяет FK-измерение
на JOIN, B1 добавляет top-N). Замерено на текущем коде: для auto-overview совпадают все 8
чартов, для LLM-спеки с FK-измерением или контролом с дефолтом — расходятся. Переиспользовать
evidence между разными стейтментами нельзя: Advisor получил бы замер запроса, который BI
никогда не выполняет (ровно тот ложный сигнал, который чинил P1-2), поэтому промах обязан
оставаться промахом. Magnitude/insights берут ключ тем же `generate_chart_sql(query)`
(default args), что pipeline при gate OWN/per-chart SQL.

Инвариант 3 не двигается: `guard_sql` (SELECT-only разбор) и LIMIT-прогон остаются
безусловными, а попадание в plan-кэш означает, что DWH уже разобрал, отрезолвил и проверил
права на **этот самый** стейтмент. `ok` выводится консервативно — «движок отказал» и
«движок не вернул ничего пригодного» одинаково дают промах, так что худшее следствие —
тот же лишний `EXPLAIN`, что был до кэша. Trial capture **добавляет ноль** DWH-проходов:
он только сохраняет то, что LIMIT-прогон уже вернул.

Время жизни — один вызов сборки; ничего не персистится. В долгоживущий Advisor `serve`
кэш намеренно не вшит (preview и build там — разные запросы). Замер приёмки D-2 §3 на
8-чартовом auto-overview: **25 → 17 проходов в DWH, 3,1 → 2,1 на чарт**. Magnitude-пробы
Superset — отдельная поверхность (D-2 §5): после D-1 экономия только на OWN-чартах;
SOURCE оставляют probe.

### 3.20 Filterable dataset / D-1 (вариант A, 2026-07-20)

Native-фильтр дашборда — это WHERE по колонке датасета чарта. Пока каждый чарт сидел
на своём уже-агрегированном virtual dataset, период/магазин двигали только те срезы,
в grain которых колонка оставалась — KPI и чужие разрезы молча игнорировали контрол.
D-1 (вариант A, gate 2026-07-20) делит спеку на две роли датасета и переносит
агрегацию выразимых чартов в BI.

**Роли SOURCE / OWN и expressibility-классификатор.** `agent/dataset_plan.py::plan_datasets`
читает только IR-признаки (инвариант 1 — без LLM):
- **SOURCE** — меры выразимы как BI-side aggregate (`SUM/AVG/COUNT…` по сырой колонке,
  ratio `num/den` как один adhoc `sqlExpression`). Чарт рендерится с **общего**
  semantic-grain датасета витрины; агрегация — в form_data.
- **OWN** — `inexpressible_reason` непуст: window-transform, scalar period-compare,
  `raw_sql`, histogram `bins`. Остаётся per-chart агрегированный датасет с
  запечённым default-периодом; preview честно помечает «фильтр не влияет».

Откат ratio на SOURCE при live-провале — одна строка в `inexpressible_reason`
(flip-тест offline уже есть); остальные невыразимости не импровизируются.

**Форма source-датасета.** `generate_source_sql` — SELECT всех колонок витрины +
label-JOIN'ов чартов, **без** `GROUP BY` / `WHERE` / `LIMIT`. Default-период
приходит как default native time-фильтра (не SQL WHERE), top-N — `series_limit` в
form_data. Один shared dataset на mart на дашборд (`plan.source_tables`);
SOURCE-чарты одной витрины делят datasource id.

**Схема alias.** Колонки витрины — bare (`date`, `revenue`). Joined-ref
`schema.table.col` → `{table}_{col}` (`dm.stores.name` → `stores_name`,
`dm.products.name` → `products_name`). Уникальность — `collect_source_aliases` /
`SourceAliasCollisionError` на plan/SQL time (не тихий ambiguous GROUP BY).
Тот же alias в form_data groupby/x_axis (`from_source=True`) и в target native
select-фильтра.
...
гейтит shared source SQL **один раз на mart** и пропускает per-chart SQL для
SOURCE-чартов (BI их не выполняет). DataLens и любой non-Superset target
сохраняют pre-D-1 per-chart `EXPLAIN`+LIMIT — D-1 source path там не реализован,
дыра инварианта 3 закрыта явно, не «заодно».

**Magnitude probe на SOURCE.** RU-единицы KPI/оси (`млрд`/`млн`, не d3 `236G`)
требуют live `POST /api/v1/chart/data`. OWN: `MAX("sum_revenue")` по
уже-агрегированному датасету. SOURCE: `_adhoc_metric(..., from_source=True)` →
`SUM("revenue")`; для grouped chart — `groupby` по source-alias dim +
`orderby: [[metric_dict, False]]` + `row_limit: 1` (tallest series point).
Best-effort: сбой пробы → default format, build не ломается.

**Живая приёмка.** `tests/test_d1_acceptance.py` (`pytest -m integration`, CI
integration-job рядом с contract): build KPI + monthly trend + bar по
`stores_name` + ratio → `chart/data` с `time_range` / `filters` на `stores_name`
→ сверка с ClickHouse; soft-collect period + store + declared assumptions
(ratio render, `time_grain_sqla` P1M/P1W, magnitude/orderby, nfc bind). Offline —
payload-shape asserts тех же контрактов без стенда.

## 4. Безопасность

- DWH: отдельная **read-only роль**, видит только DM-схемы.
- SQL-guard: sqlglot-парсинг, только `SELECT`, запрет DDL/DML/множественных стейтментов; timeout; принудительный `LIMIT` на валидационных прогонах. Табличные функции — **denylist** (`_FORBIDDEN_TABLE_FUNCS`: remote/объектные стораджи/lakehouse-форматы + RBAC-слепые локальные `merge`/`dictionary`/`executable`; C-1 2026-07-18) поверх трёх форм AST (Anonymous/Func/неквалифицированный Table — sqlglot парсит часть вызовов как таблицу). Кандидат на инверсию в **allowlist** («только известные скалярные/агрегатные функции»), если денилист начнёт отставать от новых табличных функций ClickHouse.
- LLM получает: схему и структурные метаданные всегда; любые **значения данных** (top-N низкокардинальных колонок) — только при **явном** `AUTO_BI_SEND_SAMPLES=true` (default **`false`**, plan_sol step 2 / audit P0-2: clean install не шлёт DWH values внешнему провайдеру). Дополнительно: `classification` на таблице/колонке (`public` / `internal` / `confidential` / `restricted`); samples допускаются только для `public`/`internal` (колонка перекрывает таблицу; unset → `internal`). `confidential`/`restricted` не уходят в LLM даже при opt-in. Free-text (description, synonyms, top-values) санитизируется (control chars, caps) и рендерится внутри untrusted-envelope (`render_model` / `prompt_data.py`). Значения остаются в локальном `semantic/model.yaml`; флаг управляет только outbound. Миграция: `docs/MIGRATION_SEND_SAMPLES.md`.
