...
возможные графики»): KPI по каждой мере → динамика по времени → топ-N разрезы (включая
атрибуты смежных таблиц через JOIN: город / категория / формат) → доля (pie) → детализация.
