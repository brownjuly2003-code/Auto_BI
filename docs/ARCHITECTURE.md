...
- LLM получает: схему, метаданные, top-N значений низкокардинальных колонок. Сэмплы строк — под флагом `AUTO_BI_SEND_SAMPLES` (default `true`, т.к. GraceKelly локальный и трафик идёт только в Anthropic API; выключается одним флагом для чувствительных DM).
