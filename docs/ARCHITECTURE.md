...
- **PyPI (X-2, подготовлено).** Job `pypi` в `release.yml` (параллелен docker/GH-release-job'у, независим от него) публикует sdist+wheel через [Trusted Publishing](https://docs.pypi.org/trusted-publishers/) — OIDC (`permissions: id-token: write`, `environment: pypi`, `pypa/gh-action-pypi-publish`), долгоживущий токен в секретах не нужен вовсе. Имя `auto-bi` на PyPI свободно (проверено 2026-07-07; `autobi` занят чужим проектом — это другое нормализованное имя, не конфликт). Метаданные пакета (license/authors/classifiers/urls/keywords) — в `pyproject.toml`; `uv build` + `twine check` PASS, wheel ставится в чистый venv (`auto_bi --version` работает). **Одноразовый шаг владельца ДО первого публикующего тега:** на pypi.org → Publishing → добавить pending trusted publisher (project `auto-bi`, owner `brownjuly2003-code`, repo `Auto_BI`, workflow `release.yml`, environment `pypi`). Пока он не настроен, `pypi`-job падает OIDC-ошибкой, не задевая остальной релиз.
