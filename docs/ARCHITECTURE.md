...
`PlanCache` (`agent/query_plan.py`) — мемо на **один вызов сборки**: Advisor записывает
результат своего плана, guard **только читает** и пропускает собственный `EXPLAIN`, если
этот же стейтмент уже спланирован без ошибки. Кэш не ходит в DWH сам и не импортирует ни
advisor, ни guard — слои остаются однонаправленными.

**Ключ — точный текст SQL, и это несущее.** Потребители смотрят на РАЗНЫЕ запросы:
...
безусловными, а попадание в кэш означает, что DWH уже разобрал, отрезолвил и проверил права
на **этот самый** стейтмент. `ok` выводится консервативно — «движок отказал» и «движок не
вернул ничего пригодного» одинаково дают промах, так что худшее следствие — тот же лишний
`EXPLAIN`, что был до кэша.

Время жизни — один вызов сборки; в долгоживущий Advisor `serve` кэш намеренно не вшит
(preview и build там — разные запросы, а оценка точечна во времени). Замер приёмки на
8-чартовом auto-overview: **25 → 17 проходов в DWH, 3,1 → 2,1 на чарт**; 6 magnitude-проб
Superset — отдельная поверхность (D-2 §5 п.2), в этот счёт не входят.
