...
### B5 — big_number KPI клипает крупные агрегаты — P2 — ✅ ЗАКРЫТ (2026-06-25, компактный формат, live-verified Superset+DataLens)

**РЕЗУЛЬТАТ (по quality-pass через dashboard-craft/kpi-design скиллы):** правило craft §4 «масштабировать
крупные суммы» зашито в рецепт+оба адаптера. Новый BI-agnostic предикат `ir.spec.is_compact_number`
(SUM/COUNT/COUNT_DISTINCT → компакт; AVG/MIN/MAX → полная точность). Superset: `y_axis_format`/
`number_format`/`column_config` d3 `.3~s` (API-verified чарт 154 = "236G"). DataLens: поле `formatting`
{unit:auto, precision:1} в `chart_config._field_item` (live-verified: KPI **236,1B / 115,0M / 210,0M**,
оси баров/line и метки pie тоже компактны, консоль 0 err, скрин `autobi_auto_datalens_compact_full.png`).
Заодно: чистка технического грейна из заголовка (`autospec._clean_title`). pytest 404, ruff/black clean.
**Остаточная косметика (минор, не блокер):** (а) DataLens-стенд рендерит SI-суффикс "B/M", не "млрд/млн"
(locale-bound — RU-слова потребовали бы locale/postfix-работы); (б) KPI-сабтайтл в DataLens всё ещё
сырой алиас `sum_revenue` (виджет `metric` показывает field-title; человеческая подпись = отдельный
заход — `fakeTitle`/hide-caption, реверс на стенде); (в) узкие KPI-плитки дают слабый скролл-аффорданс,
но компактное число влезает. Диагноз/история ниже сохранены для контекста.

#### B5 — диагноз (исходный, до фикса)
...
**Обновление 2026-06-25 (live-проверка + quality-pass авто-режима):** **B5** ✅ ЗАКРЫТ — компактный
формат чисел зашит в рецепт+оба адаптера через dashboard-craft/kpi-design скиллы, live-verified
(Superset API `.3~s`; DataLens KPI 236,1B/115,0M/210,0M + оси + pie, консоль 0 err) + чистый заголовок.
Остаточная косметика (B/M vs млрд, сырой KPI-сабтайтл в DataLens) — минор, см. §B5. Остальные 8 чартов
авто-обзора live-верифицированы корректными. **Итог адекватности: B1–B5 ✅ ЗАКРЫТЫ.**
