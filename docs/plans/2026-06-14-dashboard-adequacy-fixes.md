...
### B3 — Джойн id→имя для интерпретируемости — BI-agnostic, P2 — ⏸️ OWNER-DEFERRED (2026-06-15, решение делегировано → отложен)

**РЕШЕНИЕ (2026-06-15, по «Auto_BI продолжи» /auto).** На прямой вопрос «как двигать B3»
владелец ответил «решай сам» → решение: **B3 отложен, backlog адекватности закрыт для
автономного Windows-scope.** Оба пути блокируются верификационными гейтами, которые автономная
Opus/Windows-сессия не закрывает по планке качества проекта (каждый предыдущий B-пункт закрывался
live-visual или resolved-by-verification):
- **Детерминированный** «group-by-id, показывать имя» = крупная правка IR + Superset + DataLens
  адаптеров; рендеринг визуально верифицируется ТОЛЬКО на живом стенде (DataLens на Mac; Superset-стенда
  на Windows нет) → unit-тесты SQL/IR не дотягивают до планки «не шипить рендеринг без visual-verify».
  Группировка по имени (без id) — correctness-риск (слияние строк при неуникальных именах), отвергнут.
- **Промпт** = off-model по конвенции (промпт-инжиниринг не на Opus/auto → Fable/ручная сессия) + eval
  подтвердит лишь отсутствие регрессии, не улучшение (join-поведения нет в golden; реальный эффект —
  только live-LLM-прогон).

Дашборды функционально корректны (B1 top-N + B2 категориальная ось + авто-масштаб); B3 — читаемость,
не корректность. **Возобновление дёшево:** (a) живой DataLens-стенд (Mac, `docker start auto_bi_*` +
туннель) для детерминированного варианта; ЛИБО (b) Fable/ручная сессия + прогон `auto_bi eval` для
промпт-варианта. Запись решения — docs-only, код/тесты не тронуты (pytest 351 без изменений).
...
4. ~~**B3-детерминированный** (авто-join)~~ + ~~**B3-промпт**~~ — ⏸️ OWNER-DEFERRED (2026-06-15, «решай сам»):
   оба пути за верификационным гейтом (стенд для рендеринга / Fable+eval для промпта), backlog адекватности
   закрыт для автономного Windows-scope. Возобновление — при живом стенде ЛИБО Fable-сессии (см. §B3).
   ~~**B4**~~ — ✅ ЗАКРЫТ (resolved-by-verification).

**Итог backlog'а адекватности (2026-06-15):** B1 ✅ · B2 ✅ · B4 ✅ · B3 ⏸️ owner-deferred. Автономный
Windows-scope исчерпан.
