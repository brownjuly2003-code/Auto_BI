...
- [x] **Insight-слой «Что видно» (2026-06-26, vector B):** `auto_bi/agent/insights.py::analyze_spec` — детерминированный read-only проход поверх построенного авто-обзора → тренд %, лидер+концентрация, наибольшая доля, аномалия; ОТДЕЛЬНОЙ поверхностью, НЕ в дашборде. Без LLM, best-effort. CLI печатает блок под `build --auto`. 13 unit-тестов (фейковый `RunQuery`, offline). ARCHITECTURE §3.10. Остаток: API-эндпоинт + панель в web UI (демо-эффект), live-сверка на стенде (опц., проход детерминирован).
