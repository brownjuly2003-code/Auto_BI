...
## Открыто (не делала — решение Юли)

`explain_high_scan_fraction` на compare-KPI советует «narrow the time range or add a filter»,
хотя фильтр уже стоит, а расширение окна — by design. Находка честная (запрос правда читает
29M строк), но совет неактуален. Варианты: (а) оставить; (б) правило знает про period-compare
и молчит; (в) отдельный текст совета. Пороги/дизайн правила — не мой вызов.
